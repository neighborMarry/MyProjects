...
/*
 * error_response: return error msg to client when error occurs
 */
void error_response(int fd, const char *msg) {
    char buf[MAXLINE];
    char body[MAXLINE];
    size_t n;
    body[0] = '\0';
    buf[0] = '\0';
    strcat(body, "<html><body>");
    strcat(body, msg);
    ...
    n = strlen(buf);
    dbg_printf("error response: %s\n", buf);
    if ((size_t)rio_writen(fd, buf, n) != n) {
        printf("error response write error\n");
    }
}
